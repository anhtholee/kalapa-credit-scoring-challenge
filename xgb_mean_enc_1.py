...
def main():
    ...
    for i, (train_index, test_index) in enumerate(folds):
        ...
        for seed in tqdm(range(n_seeds)):
            ...
            # print(f"seed {seed}")
            # print("Mean encoding...")
            Xtrain, Xtest = mean_encoding(Xtrain, ytrain, Xtest, ['field_3', 'field_22', 'field_9', 'field_19', 'macv', 'province_district'])
            np.random.seed(seed=int(time.time()))
            clf = xgb.XGBClassifier(**xgb_params)
            clf.fit(
                Xtrain, 
            ...
            Xtrain, X_test = mean_encoding(Xtrain, ytrain, X_test, ['field_3', 'field_22', 'field_9', 'field_19', 'macv', 'province_district'])
